...
interface NotificacionItem { id: number; usuario_id: string | null; titulo: string; mensaje: string; tipo: string; leido: boolean; fecha_envio?: string | null; perfil?: PerfilRelacionado | null; }
...
export default function NotificacionesAdmin() {
  ...
  const cargarNotificaciones = async () => {
    try {
      ...
      const { data, error } = await supabase.from('notificaciones').select('id, usuario_id, titulo, mensaje, tipo, leido, fecha_envio').order('fecha_envio', { ascending: false });
      if (error) { setError(error.message); return; }
      const lista = data || [];
      const usuariosIds = [...new Set(lista.map((item: any) => item.usuario_id).filter((id: any): id is string => !!id))];
      let perfilesMap = new Map<string, PerfilRelacionado>();
      if (usuariosIds.length > 0) {
        const { data: perfilesData, error: perfilesError } = await supabase.from('perfiles').select('id, nombre_completo, correo, rol').in('id', usuariosIds);
        if (perfilesError) { setError(perfilesError.message); return; }
        perfilesMap = new Map((perfilesData || []).map((p: any) => [p.id, p]));
      }
      const listaConPerfiles: NotificacionItem[] = lista.map((item: any) => ({ ...item, perfil: item.usuario_id ? perfilesMap.get(item.usuario_id) || null : null }));
      setNotificaciones(listaConPerfiles);
    } catch { setError('No se pudieron cargar las notificaciones'); } finally { setCargando(false); }
  };

  const cargarUsuarios = async () => {
    const { data, error } = await supabase.from('perfiles').select('id, nombre_completo, correo, rol').eq('estado', 'activo').order('nombre_completo', { ascending: true });
    if (error) { setError(error.message); return; }
  ...
  const crearNotificacion = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('notificaciones').insert({ usuario_id: nuevoUsuarioId || null, titulo: nuevoTitulo.trim(), mensaje: nuevoMensaje.trim(), tipo: nuevoTipo, leido: false, fecha_envio: new Date().toISOString() });
      if (error) { alert('No se pudo crear la notificación: ' + error.message); return; }
      alert('Notificación creada correctamente'); cerrarModalNueva(); await cargarNotificaciones();
    } catch { alert('Ocurrió un error al crear la notificación'); } finally { setGuardandoNueva(false); }
  };
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('notificaciones').update({ titulo: formTitulo.trim(), mensaje: formMensaje.trim(), tipo: formTipo, leido: formLeido }).eq('id', notificacionEditando.id);
  ...
  const cambiarLeido = async (notificacion: NotificacionItem, valor: boolean) => {
    const { error } = await supabase.from('notificaciones').update({ leido: valor }).eq('id', notificacion.id);
                  ...
                  <tr key={n.id} style={!n.leido ? { background: 'rgba(49,151,149,0.03)' } : {}}>
                    ...
                    <td style={styles.td}>
                      {n.perfil?.nombre_completo ? (
                        <div><div style={{ fontWeight: 500 }}>{n.perfil.nombre_completo}</div><div style={{ fontSize: '11px', color: '#94A3B8' }}>{n.perfil.correo}</div></div>
