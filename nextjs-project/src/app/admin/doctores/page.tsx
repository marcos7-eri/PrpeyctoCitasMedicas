...
export default function DoctoresAdmin() {
  ...
  const cargarDoctores = async () => {
    try {
      ...
      const { data, error } = await supabase.from('doctores').select(`id, perfil_id, especialidad_id, numero_licencia, anios_experiencia, costo_consulta, biografia, perfiles(id, nombre_completo, correo, telefono, estado), especialidades(id, nombre)`);
      if (error) { setError(error.message); return; }
      setDoctores((data as unknown as DoctorItem[]) || []);
    } catch { setError('No se pudieron cargar los doctores'); } finally { setCargando(false); }
  };

  const cargarEspecialidades = async () => {
    const { data } = await supabase.from('especialidades').select('id, nombre').order('nombre', { ascending: true });
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('doctores').update({ numero_licencia: formLicencia, anios_experiencia: formExperiencia ? Number(formExperiencia) : null, costo_consulta: formCosto ? Number(formCosto) : null, biografia: formBiografia, especialidad_id: formEspecialidadId ? Number(formEspecialidadId) : null }).eq('id', doctorEditando.id);
      if (error) { alert('No se pudo actualizar el doctor: ' + error.message); return; }
      await cargarDoctores(); cerrarEdicion();
    } catch { alert('Ocurrió un error al guardar los cambios'); } finally { setGuardandoEdicion(false); }
  };
  ...
  const guardarNuevoDoctor = async () => {
    ...
    try {
      ...
      if (!session?.access_token) { alert('No se encontró la sesión del usuario administrador'); return; }
      const { data, error } = await supabase.functions.invoke('crear-doctor', {
        body: { nombre_completo: nuevoNombre, correo: nuevoCorreo, telefono: nuevoTelefono, contrasena: nuevaContrasena, estado: 'activo', especialidad_id: nuevaEspecialidadId || null, numero_licencia: nuevaLicencia, anios_experiencia: nuevaExperiencia || null, costo_consulta: nuevoCosto || null, biografia: nuevaBiografia },
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (error) { alert('Error al crear doctor: ' + JSON.stringify(error)); return; }
      if (data?.error) { alert('Error: ' + data.error); return; }
      alert('Doctor creado correctamente'); cerrarNuevo(); await cargarDoctores();
    } catch (err) { console.error(err); alert('Ocurrió un error al crear el doctor'); } finally { setGuardandoNuevo(false); }
  };
