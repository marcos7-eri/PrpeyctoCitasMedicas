...
export default function CitasAdmin() {
  ...
  const cargarCitas = async () => {
    try {
      ...
      const { data, error } = await supabase.from('citas').select(`id, paciente_id, doctor_id, fecha, hora_inicio, hora_fin, estado, motivo, notas, motivo_cancelacion, creado_en, pacientes(id, perfiles(nombre_completo, correo, telefono)), doctores(id, perfiles(nombre_completo, correo, telefono), especialidades(nombre))`).order('fecha', { ascending: false });
      if (error) { setError(error.message); return; }
      setCitas((data as unknown as CitaItem[]) || []);
    } catch { setError('No se pudieron cargar las citas'); } finally { setCargando(false); }
  };

  const cargarPacientes = async () => {
    const { data } = await supabase.from('pacientes').select(`id, perfiles(nombre_completo, correo)`);
    setPacientes((data as unknown as PacienteOption[]) || []);
  };

  const cargarDoctores = async () => {
    const { data } = await supabase.from('doctores').select(`id, perfiles(nombre_completo, correo), especialidades(nombre)`);
  ...
  const crearCita = async () => {
    ...
    try {
      ...
      const payload: any = { paciente_id: nuevoPacienteId, doctor_id: nuevoDoctorId, fecha: nuevaFecha, hora_inicio: nuevaHoraInicio, hora_fin: nuevaHoraFin || null, estado: 'pendiente', motivo: nuevoMotivo || null, notas: nuevasNotas || null };
      if (creadorId) payload.creado_por = creadorId;
      const { error } = await supabase.from('citas').insert(payload);
      if (error) { alert('No se pudo crear la cita: ' + error.message); return; }
      alert('Cita creada correctamente'); cerrarModalNueva(); await cargarCitas();
    } catch { alert('Ocurrió un error al crear la cita'); } finally { setGuardandoNueva(false); }
  };
  ...
  const cambiarEstado = async (cita: CitaItem, nuevoEstado: string) => {
    ...
    } catch (e) { console.error(e); alert('Ocurrió un error al actualizar la cita'); } finally { setProcesandoId(null); }
  };
  ...
  const cancelarCita = async (cita: CitaItem) => {
    ...
    try {
      setProcesandoId(cita.id);
      const { error } = await supabase.from('citas').update({ estado: 'cancelada', motivo_cancelacion: motivo || null }).eq('id', cita.id);
    ...
    } catch (e) { console.error(e); alert('Ocurrió un error al cancelar la cita'); } finally { setProcesandoId(null); }
  };
