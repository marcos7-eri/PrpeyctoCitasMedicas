...
export default function DashboardDoctor() {
  ...
  useEffect(() => { if (doctorId) cargarDashboard(); }, [doctorId]);

  const obtenerDoctorId = async () => {
    try {
      const { data: userData } = await supabase.auth.getUser();
      if (userData.user) {
        const { data: doctorData } = await supabase.from('doctores').select('id').eq('perfil_id', userData.user.id).single();
  ...
  const cargarDashboard = async () => {
    ...
    try {
      ...
      const [citasHoyRes, citasPendientesRes, pacientesRes, notificacionesRes, citasProximasRes] = await Promise.all([
        supabase.from('citas').select('*', { count: 'exact', head: true }).eq('doctor_id', doctorId).eq('fecha', hoy),
        supabase.from('citas').select('*', { count: 'exact', head: true }).eq('doctor_id', doctorId).eq('estado', 'pendiente'),
        supabase.from('citas').select('paciente_id').eq('doctor_id', doctorId),
        supabase.from('notificaciones').select('id', { count: 'exact', head: false }).eq('leido', false),
        supabase.from('citas').select('id, fecha, hora_inicio, estado, motivo, pacientes(perfiles(nombre_completo))').eq('doctor_id', doctorId).gte('fecha', hoy).order('fecha', { ascending: true }).order('hora_inicio', { ascending: true }).limit(5),
      ]);
      const pacientesUnicos = new Set((pacientesRes.data || []).map((c: any) => c.paciente_id).filter(Boolean));
      ...
      setStats({ citasHoy: citasHoyRes.count || 0, citasPendientes: citasPendientesRes.count || 0, totalPacientes: pacientesUnicos.size, notificacionesNoLeidas: notificacionesRes.count || 0 });
