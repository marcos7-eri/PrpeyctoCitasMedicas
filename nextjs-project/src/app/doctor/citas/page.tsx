...
export default function CitasDoctor() {
  ...
  const obtenerDoctorId = async () => {
    try {
      ...
      const { data: doctorData, error } = await supabase.from('doctores').select('id').eq('perfil_id', userData.user.id).single();
      if (error || !doctorData) { setCargando(false); return; }
  ...
  const cargarPacientes = async () => {
    try {
      const { data, error } = await supabase.from('pacientes').select('id, perfil_id, perfiles(nombre_completo, correo)').order('id', { ascending: true });
      if (error) throw error;
      setPacientes((data || []).map((p: any) => ({ id: p.id, perfil_id: p.perfil_id, nombre_completo: p.perfiles?.nombre_completo || 'Paciente', correo: p.perfiles?.correo || '' })));
  ...
  const cargarCitas = async () => {
    ...
    try {
      setCargando(true);
      const { data, error } = await supabase.from('citas').select('id, paciente_id, fecha, hora_inicio, hora_fin, estado, motivo, pacientes(perfiles(nombre_completo, correo))').eq('doctor_id', doctorId).order('fecha', { ascending: true }).order('hora_inicio', { ascending: true });
      if (error) { console.error('Error cargando citas:', error); return; }
      setCitas((data || []).map((cita: any) => ({ id: cita.id, paciente_id: cita.paciente_id, fecha: cita.fecha, hora_inicio: cita.hora_inicio, hora_fin: cita.hora_fin, estado: cita.estado, motivo: cita.motivo, paciente_nombre: cita.pacientes?.perfiles?.nombre_completo || 'Paciente', paciente_correo: cita.pacientes?.perfiles?.correo || '' })));
  ...
  const guardarNuevaCita = async () => {
    ...
    if (diaSemana === 0) { alert('El doctor solo puede programar citas de lunes a sábado'); return; }
    try {
      setGuardando(true);
      const { data: horariosData, error: errorHorarios } = await supabase.from('horarios').select('id, dia_semana, hora_inicio, hora_fin, duracion_cita, activo').eq('doctor_id', doctorId).eq('dia_semana', diaSemana).eq('activo', true);
      if (errorHorarios) throw errorHorarios;
      const horarios = (horariosData as HorarioDoctor[]) || [];
      if (horarios.length === 0) { alert('No tienes horarios activos para ese día'); return; }
      ...
      const { data: citasExistentes, error: errorCitas } = await supabase.from('citas').select('id, hora_inicio, hora_fin, estado').eq('doctor_id', doctorId).eq('fecha', nuevaFecha).neq('estado', 'cancelada');
      if (errorCitas) throw errorCitas;
      const existeCruce = (citasExistentes || []).some((cita: any) => {
        const inicioExistente = cita.hora_inicio.substring(0, 5);
        const finExistente = cita.hora_fin ? cita.hora_fin.substring(0, 5) : sumarMinutos(inicioExistente, horarioValido.duracion_cita);
      ...
      const { error } = await supabase.from('citas').insert({ doctor_id: doctorId, paciente_id: nuevoPacienteId, fecha: nuevaFecha, hora_inicio: nuevaHoraInicio, hora_fin: nuevaHoraFin, motivo: nuevoMotivo.trim() || null, estado: 'pendiente' });
      if (error) throw error;
