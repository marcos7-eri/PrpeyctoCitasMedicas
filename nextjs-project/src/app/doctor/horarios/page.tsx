...
export default function HorariosDoctor() {
  ...
  const cargarHorarios = async () => {
    ...
    try {
      setCargando(true);
      const { data, error } = await supabase.from('horarios').select('*').eq('doctor_id', doctorId).order('dia_semana', { ascending: true });
  ...
  const agregarHorario = async () => {
    ...
    try {
      setGuardando(true);
      const { error } = await supabase.from('horarios').insert({ doctor_id: doctorId, dia_semana: nuevoDia, hora_inicio: nuevaHoraInicio, hora_fin: nuevaHoraFin, activo: true });
      if (error) throw error;
      setMostrarModal(false);
      await cargarHorarios();
    } catch (error) { console.error('Error agregando horario:', error); alert('Error al agregar horario'); } finally { setGuardando(false); }
  };

  const toggleHorario = async (horario: Horario) => {
    try {
      const { error } = await supabase.from('horarios').update({ activo: !horario.activo }).eq('id', horario.id);
      if (error) throw error;
      await cargarHorarios();
    } catch (error) { console.error('Error actualizando horario:', error); }
  };
  ...
  const eliminarHorario = async (id: string) => {
    ...
    try {
      ...
      if (error) throw error;
      await cargarHorarios();
    } catch (error) { console.error('Error eliminando horario:', error); }
  };
