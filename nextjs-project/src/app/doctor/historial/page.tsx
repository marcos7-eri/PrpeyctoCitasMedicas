...
export default function HistorialDoctor() {
  ...
  const cargarHistorial = async () => {
    ...
    try {
      setCargando(true);
      const { data, error } = await supabase.from('citas').select('id, fecha, hora_inicio, motivo, estado, pacientes(perfiles(nombre_completo))').eq('doctor_id', doctorId).in('estado', ['completada', 'cancelada']).order('fecha', { ascending: false });
      if (error) throw error;
      setHistorial((data || []).map((cita: any) => ({ id: cita.id, fecha: cita.fecha, hora_inicio: cita.hora_inicio, motivo: cita.motivo, estado: cita.estado, paciente_nombre: cita.pacientes?.perfiles?.nombre_completo || 'Paciente' })));
