...
export default function PacientesDoctor() {
  ...
  const cargarPacientes = async () => {
    ...
    try {
      setCargando(true);
      const { data, error } = await supabase.from('citas').select('paciente_id, pacientes(perfiles(id, nombre_completo, correo, telefono))').eq('doctor_id', doctorId);
      if (error) throw error;
      const pacientesMap = new Map<string, Paciente>();
      (data || []).forEach((cita: any) => {
        const perfil = cita.pacientes?.perfiles;
        if (perfil) {
          if (!pacientesMap.has(perfil.id)) {
            pacientesMap.set(perfil.id, { id: perfil.id, nombre_completo: perfil.nombre_completo, correo: perfil.correo, telefono: perfil.telefono, total_citas: 1 });
          } else {
            pacientesMap.get(perfil.id)!.total_citas++;
          }
        }
      });
