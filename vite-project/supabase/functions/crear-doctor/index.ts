...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (perfilError || !perfilAdmin || perfilAdmin.rol !== 'admin') {
      return new Response(JSON.stringify({ error: 'Solo un admin puede crear doctores' }), {
        status: 403,
        headers: corsHeaders,
      });
    }
    ...
    if (updatePerfilError) {
      return new Response(
        JSON.stringify({
          error:
            'Usuario auth creado, pero falló la actualización del perfil: ' +
    ...
    const { error: doctorError } = await supabaseAdmin.from('doctores').insert({
      perfil_id: userId,
      especialidad_id: especialidad_id ? Number(especialidad_id) : null,
      numero_licencia: numero_licencia || null,
      anios_experiencia: anios_experiencia ? Number(anios_experiencia) : null,
      costo_consulta: costo_consulta ? Number(costo_consulta) : null,
    ...
    if (doctorError) {
      return new Response(
        JSON.stringify({
          error: 'Usuario creado, pero falló el registro del doctor: ' + doctorError.message,
        }),
        {
          status: 500,
          headers: corsHeaders,
