...
export default function PacientesAdmin() {
  ...
  const cargarPacientes = async () => {
    try {
      ...
      const data = await api.get<PacienteItem[]>('/pacientes');
      setPacientes(data || []);
    } catch (e: any) { setError(e.message || 'No se pudieron cargar los pacientes'); } finally { setCargando(false); }
  };
  ...
  const cambiarEstadoPaciente = async (paciente: PacienteItem) => {
    if (!paciente.perfil_id) return;
    const nuevoEstado = paciente.perfiles?.estado === 'activo' ? 'inactivo' : 'activo';
    try {
      setProcesandoId(paciente.id);
      await api.put(`/usuarios/${paciente.perfil_id}`, { estado: nuevoEstado });
      await cargarPacientes();
    } catch (e: any) { alert('No se pudo actualizar el estado: ' + e.message); } finally { setProcesandoId(null); }
  };
  ...
  const guardarEdicion = async () => {
    if (!pacienteEditando || !pacienteEditando.perfil_id) return;
    try {
      setGuardandoEdicion(true);
      await Promise.all([
        api.put(`/usuarios/${pacienteEditando.perfil_id}`, { nombre_completo: formNombre, telefono: formTelefono, estado: formEstado }),
        api.put(`/pacientes/${pacienteEditando.id}`, {
    ...
    } catch (e: any) { alert('No se pudo actualizar el paciente: ' + e.message); } finally { setGuardandoEdicion(false); }
  };

  const verDetalle = async (paciente: PacienteItem) => {
    setPacienteDetalle(paciente); setCitasDetalle([]); setCargandoDetalle(true);
    try {
      const data = await api.get<CitaResumen[]>(`/citas?paciente_id=${paciente.id}`);
      setCitasDetalle(data || []);
    } catch (e: any) { alert('No se pudieron cargar las citas: ' + e.message); } finally { setCargandoDetalle(false); }
  };
