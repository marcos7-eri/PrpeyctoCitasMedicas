...
function AdminLayoutInner({ children }: AdminLayoutProps) {
  ...
  const nombreMostrar = perfil?.nombre_completo || 'Administrador';
  const rolMostrar = perfil?.rol === 'admin' ? 'Administrador' : perfil?.rol === 'doctor' ? 'Doctor' : perfil?.rol === 'paciente' ? 'Paciente' : 'admin';
  ...
  const menuAdmin = [
    ...
    { nombre: 'Estadísticas', ruta: '/admin/estadisticas', icono: 'M9 19V13M12 19V7M15 19V11M3 3H21M3 21H21' },
    { nombre: 'Reportes',    ruta: '/admin/reportes',    icono: 'M12 10V16M9 13H15M5 3H19C20.1046 3 21 3.89543 21 5V19C21 20.1046 20.1046 21 19 21H5C3.89543 21 3 20.1046 3 19V5C3 3.89543 3.89543 3 5 3Z' },
    ...
    <div style={styles.container}>
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          <div>
            <h1 style={styles.pageTitle}>{menuAdmin.find(m => m.ruta === pathname)?.nombre || 'Admin'}</h1>
            <p style={styles.pageSubtitle}>Panel de administración</p>
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  orb1: { position: 'absolute', top: '-10%', right: '-5%', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(49,151,149,0.25) 0%, transparent 70%)', filter: 'blur(60px)', animation: 'float 15s ease-in-out infinite' },
  orb2: { position: 'absolute', bottom: '-10%', left: '-5%', width: '600px', height: '600px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(79,70,229,0.15) 0%, transparent 70%)', filter: 'blur(60px)', animation: 'float 20s ease-in-out infinite reverse' },
  orb3: { position: 'absolute', top: '40%', left: '30%', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(236,72,153,0.08) 0%, transparent 70%)', filter: 'blur(60px)', animation: 'float 25s ease-in-out infinite' },
  sidebar: { width: '280px', background: 'rgba(20, 24, 35, 0.8)', backdropFilter: 'blur(20px)', borderRight: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', padding: '24px 20px', position: 'relative', zIndex: 10 },
  logoArea: { display: 'flex', alignItems: 'center', gap: '12px', paddingBottom: '24px', marginBottom: '24px', borderBottom: '1px solid rgba(255,255,255,0.08)' },
  logoIcon: { width: '48px', height: '48px', background: 'rgba(49,151,149,0.15)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  ...
  navItem: { display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'transparent', border: 'none', borderRadius: '12px', color: '#94A3B8', fontSize: '14px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.2s ease', position: 'relative' },
  navItemActive: { display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'rgba(49,151,149,0.15)', border: 'none', borderRadius: '12px', color: '#319795', fontSize: '14px', fontWeight: '500', cursor: 'pointer', position: 'relative' },
  navIcon: { display: 'flex', alignItems: 'center', justifyContent: 'center', width: '20px' },
  activeIndicator: { position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: '3px', height: '20px', background: '#319795', borderRadius: '3px' },
  sidebarFooter: { marginTop: 'auto', paddingTop: '24px', borderTop: '1px solid rgba(255,255,255,0.08)' },
  logoutBtn: { display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.05)', border: 'none', borderRadius: '12px', color: '#FCA5A5', fontSize: '14px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.2s ease' },
  main: { flex: 1, padding: '24px 32px', position: 'relative', zIndex: 10, overflowY: 'auto' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px', padding: '20px 24px', background: 'rgba(20, 24, 35, 0.6)', backdropFilter: 'blur(12px)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)' },
  ...
  avatar: { width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, #319795 0%, #1a5a58 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFFFFF', fontWeight: '600', fontSize: '16px', overflow: 'hidden', border: '2px solid rgba(255,255,255,0.1)' },
  avatarImage: { width: '48px', height: '48px', borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,0.1)', display: 'block' },
