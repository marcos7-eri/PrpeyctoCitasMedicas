...
export default function UsuariosAdmin() {
  ...
  const [nuevoRol, setNuevoRol] = useState<'admin' | 'paciente'>('paciente');
  ...
  const cambiarEstadoUsuario = async (usuario: UsuarioPerfil) => {
    const nuevoEst = usuario.estado === 'activo' ? 'inactivo' : 'activo';
    try {
      setProcesandoId(usuario.id);
      await api.put(`/usuarios/${usuario.id}`, { estado: nuevoEst });
      setUsuarios((prev) => prev.map((u) => u.id === usuario.id ? { ...u, estado: nuevoEst as 'activo' | 'inactivo' } : u));
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      await api.put(`/usuarios/${usuarioEditando.id}`, { nombre_completo: formNombre, telefono: formTelefono, rol: formRol, estado: formEstado });
    ...
    } catch (e: any) { alert('No se pudo actualizar el usuario: ' + e.message); } finally { setGuardandoEdicion(false); }
  };
  ...
  const guardarNuevoUsuario = async () => {
    ...
    try {
      ...
      await api.post('/usuarios', { nombre: nuevoNombre, correo: nuevoCorreo, telefono: nuevoTelefono, password: nuevaContrasena, rol: nuevoRol });
      alert('Usuario creado correctamente');
      setMostrarModalNuevo(false);
      setNuevoNombre(''); setNuevoCorreo(''); setNuevoTelefono(''); setNuevaContrasena('');
      await cargarUsuarios();
    } catch (e: any) { alert('Error al crear usuario: ' + e.message); } finally { setGuardandoNuevo(false); }
  };
