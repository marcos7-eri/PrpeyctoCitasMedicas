...
export default function PerfilDoctor() {
  ...
  const guardarPerfil = async () => {
    ...
    try {
      ...
      await api.put(`/usuarios/${perfil.id}`, { nombre_completo: formNombre.trim(), correo: formCorreo.trim(), telefono: formTelefono.trim() || null, foto_url: fotoUrlFinal });
    ...
    } catch (e: any) { alert('No se pudo guardar el perfil: ' + (e?.message || 'Error desconocido')); } finally { setGuardando(false); }
  };

  const getRolTexto = (rol: string) => ({ admin: 'Administrador', doctor: 'Doctor', paciente: 'Paciente' }[rol] || rol);
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '24px', marginTop: '24px' }}>
        <div style={styles.card}>
          ...
          <div style={styles.formGroup}><label style={styles.label}>Seleccionar imagen</label><input type="file" accept="image/*" onChange={seleccionarFoto} style={{ ...styles.input, padding: '10px' }} /></div>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.formGroup}><label style={styles.label}>Nombre completo *</label><input type="text" value={formNombre} onChange={(e) => setFormNombre(e.target.value)} style={styles.input} placeholder="Tu nombre completo" /></div>
          <div style={styles.formGroup}><label style={styles.label}>Correo electrónico *</label><input type="email" value={formCorreo} onChange={(e) => setFormCorreo(e.target.value)} style={styles.input} placeholder="tu@email.com" /></div>
          <div style={styles.formGroup}><label style={styles.label}>Teléfono</label><input type="tel" value={formTelefono} onChange={(e) => setFormTelefono(e.target.value)} style={styles.input} placeholder="+591 12345678" /></div>
